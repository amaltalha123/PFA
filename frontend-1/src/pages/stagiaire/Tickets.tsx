...
import { Person, Schedule, Download, ArrowDownward, ArrowUpward } from '@mui/icons-material';
...
const Tickets: React.FC = () => {
  ...
  const [newTicket, setNewTicket] = useState<{ Contenue: string; piece_jointe?: File | null }>({
    Contenue: '',
    piece_jointe: null,
  });
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      setNewTicket({ ...newTicket, piece_jointe: event.target.files[0] });
    }
  };
...
const handleSubmitTicket = async () => {
  ...
  formData.append('Contenue', newTicket.Contenue);
  if (newTicket.piece_jointe) {
    formData.append('piece_jointe', newTicket.piece_jointe);
  }

  try {
    const response = await axiosClient.post(`/ticket/add/${assignment.id}`, formData, {
      headers: {
      ...
      >
        ...
        <DialogContent>
          <TextField
            ...
            multiline
            rows={4}
            value={newTicket.Contenue}
            onChange={(e) => setNewTicket({ ...newTicket, Contenue: e.target.value })}
          />
          <input
            accept="*"
            style={{ display: 'none' }}
          ...
          <div className="flex-1">
            ...
            {ticket.piece_jointe && (
              <div style={{ marginTop: '8px' }}>
                ...
                >
                  {ticket.piece_jointe.substring(ticket.piece_jointe.lastIndexOf('/') + 1)}
                </Button>
              </div>
            )}
          </div>
